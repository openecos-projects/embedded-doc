...
!!! info "文档说明"
    ...
    本文档介绍 SDK 2.0 版本中 GPIO 驱动的统一 HAL 接口。该接口采用硬件抽象层设计，标准化 API 定义，具备跨平台兼容性。开发者无需关注底层寄存器与位操作，可直接调用统一接口完成通用 IO 控制。

## 概述
### 版本特性
SDK 2.0 版本的 GPIO 驱动基于统一 HAL_GPIO 接口实现，兼容标准 HAL 开发习惯。驱动支持输入/输出方向配置、电平设置与读取，并内置读写同步逻辑，提升接口易用性与数据可靠性。

### 适用范围
适用于 SDK 支持的所有平台上的通用 GPIO 控制场景，包括 LED 控制、按键检测、继电器驱动、通用 IO 扩展等。
...
*   **范围**：**0 ~ 63**，直接对应硬件寄存器中的位偏移，每一个值对应板卡上的一个物理 GPIO 引脚。
...
*   **参数说明**：
	* `gpio_id`：GPIO 组编号，用于区分不同的 GPIO 外设组。
	* `gpio_num`：引脚编号，需使用`gpio_num_t`枚举值传入。
...
*   **功能**：从 GPIO 输入寄存器读取所有引脚的电平状态，并刷新到驱动内部的软件缓存`rdr`中，为后续 gpio_hal_get_level() 调用提供有效数据。
