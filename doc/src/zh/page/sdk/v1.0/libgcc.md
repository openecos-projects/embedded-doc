...
```c title="libgcc"
uint64_t __muldi3(uint64_t u, uint64_t v);
```
...
```c title="lshrdi3.c"
uint64_t __lshrdi3(uint64_t input, int b);
```
...
## 除法与取模运算 (Division & Modulo Operations)

### 32位无符号整数除法函数 (`udivsi3`)
