...
该函数可以将单个字节载入发送缓冲区。在执行写入操作前，驱动必须确认硬件发送单元处于空闲状态。
...
!!! info "实现细节"
    ...
    *   **同步机制**：通过轮询 `LSR` 寄存器的状态位，确保数据流受控。只有当 Bit 8 为 0（即 FIFO 具备存储空间）时，才会允许后续数据写入，从而规避数据溢出风险。

### 字符串发送函数 (`hp_uart_send_str`)

该函数是针对字符串数据的自动化传输封装，通过指针遍历内存空间，来实现连续的数据输出。

```c title="hp_uart.h"
void hp_uart_send_str(const char *str);
```

**逻辑解析**

内部采用 `while (*str)` 结构检测 C 语言标准字符串结束符。它通过串行循环调用 `hp_uart_send` 接口，将应用层文本转化为底层的字节流。
