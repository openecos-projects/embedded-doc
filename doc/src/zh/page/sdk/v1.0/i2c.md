...
### 寄存器地址长度枚举 (`i2c_reg_addr_len_t`)

用于定义目标外设寄存器地址的宽度。由于不同复杂度的 I2C 设备（例如简单的 IO 扩展器与复杂的 EEPROM）寻址范围不同，该枚举函数决定了驱动在传输起始阶段发送地址字节的次数。
...
*   `pscr`：该变量直接写入预分频寄存器，用于将系统时钟分频以产生 I2C 标准频率，是用来确保总线时序同步的物理基础。
...
## 数据传输接口 (Data Transfer APIs)

### 多字节写入函数 (`i2c_write_nbyte`)

该函数可以实现完整的 I2C 写操作，包括起始位、从机地址寻址、寄存器偏移量定位及数据流写入。

```c title="i2c.h"
void i2c_write_nbyte(uint8_t addr, uint16_t reg_addr, i2c_reg_addr_len_t reg_addr_len, uint8_t *data, uint32_t len);
```
...
### 多字节读取函数 (`i2c_read_nbyte`)

该函数用于执行复合格式读取，首先通过“伪写”操作告知从机目标地址，随后切换至读取模式提取数据。

```c title="i2c.h"
void i2c_read_nbyte(uint8_t addr, uint16_t reg_addr, i2c_reg_addr_len_t reg_addr_len, uint8_t *data, uint32_t len);
```
