...
### 系统串口初始化函数 (`sys_uart_init`)

该函数用于调试串口的底层硬件配置。驱动程序通过宏 `CONFIG_SYS_UART_IP_ID` 兼容不同的 UART IP 核心，可以建立波特率发生器的基准时钟分频，并锁定数据通信的帧格式。
...
!!! info "实现细节"
    ...
    *   **物理协议锁存**：最后一步将 LC 寄存器设为 `0x03`，不仅关闭了 DLAB 访问权限以恢复数据通信，还同时定义了物理链路为 8N1 格式（即 8 位数据、无校验位、1 个停止位）。

## 数据传输接口 (Data Transfer APIs)

### 字符输出函数 (`sys_putchar`)

该接口可以将单个字节载入发送缓冲区。由于 UART 是异步慢速设备，函数在写入寄存器前必须实时监控硬件状态，防止因 CPU 写入过快导致的数据覆盖错误。
...
```c title="sys_uart.h"
void sys_putstr(const char *str);
```
